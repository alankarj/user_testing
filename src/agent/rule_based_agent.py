...
class RuleBasedAgent:
    ...
    def next(self, state):
        ...
        while not correct_ts_string_found:
            if task_state[config.TURN_STR] > 0:
                if self.params.agent_social_type != config.AGENT_SOCIAL_TYPE[0]:
                    agent_ts_string_ack, agent_cs_ack = self.reasoner_helper(self.reasoner[config.ACK_STR], key)
            agent_ts_string_other, agent_cs_other = self.reasoner_helper(self.reasoner[config.OTHER_STR], key, ack_cs=agent_cs_ack)
        ...
        return agent_ts_string_ack, agent_cs_ack, agent_ts_string_other, agent_cs_other, value

    def reasoner_helper(self, dictionary, key, ack_cs = ''):
        """
        :param dictionary: ACK/TASK Reasoner
        :param key: Triple of (prev_agent_ts_string, user_ts_string, user_cs)
        :param ack_cs: ACK CS
        :return: Tuple of (agent_ts_string, agent_cs)
        """
        ...
        alt_key = (key[0], key[1], '')
        if alt_key in config.get_agent_loop_keys():
    ...
    def social_reasoner(self, cs_list, ack_cs=''):
        params = self.params
        cs = ack_cs
